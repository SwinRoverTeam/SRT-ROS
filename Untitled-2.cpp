	...
	auto joystick_callback = [this](const sensor_msgs::msg::Joy::SharedPtr msg) -> void {
		...
		switch (mode) {
			...
			case InputMode::JOYSTICK: {
				// Left joystick steers the wheels — reverseOn already correct
				TurnLeftMotor  = false;
...
std::vector<double> XboxCtrlNode::MotorCompiler(double motor) {
	...
	if      (TurnRightMotor && !TurnLeftMotor) { driveMode = DriveMode::PIVOT_RIGHT; }
	else if (!TurnRightMotor && TurnLeftMotor) { driveMode = DriveMode::PIVOT_LEFT;  }
	else if (reverseOn)                        { driveMode = DriveMode::REVERSE;     }
	else                                       { driveMode = DriveMode::FORWARD;     }
	...
	switch (driveMode) {
		...
		default:
			// Wheels face front — drive all four motors forward
