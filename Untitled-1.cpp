...
XboxCtrlNode::XboxCtrlNode() : Node("XboxController") {
	...
	auto joystick_callback = [this](const sensor_msgs::msg::Joy::SharedPtr msg) -> void {
		...
		if (SendValBtn == 1) {
			// Only compute JoystickAlgorithm (which updates reverseOn) when LB is
			// held, so that reverseOn is not clobbered when the stick drifts back
			// to centre between presses.
			auto joystickArr = JoystickAlgorithm(left_joystick_y, left_joystick_x);
			bool reverseOnAfterJoystick = reverseOn; // snapshot after joystick
			auto gamepadArr  = JoystickAlgorithm(gamepad_y, gamepad_x);
			// Restore reverseOn to the joystick snapshot: gamepad call must not
			// silently overwrite it when joystick is the active input (and
			// vice-versa).  The final published path below will set reverseOn
			// explicitly to the value that matches the input that actually won.
			...
			if ((savedArr > joystickArr[1] + 2 || savedArr < joystickArr[1] - 2) && gamepadArr[1] == 0 && (TurnRightBtn != 1 && TurnLeftBtn != 1)) {
				...
				// joystick is active input — keep reverseOn from joystick calc
				TurnLeftMotor = false;
				...
				// gamepad is active input — recompute reverseOn from gamepad calc
...
std::vector<double> XboxCtrlNode::MotorCompiler(double motor) {
	...
	else if (TurnRightMotor && !TurnLeftMotor) {
