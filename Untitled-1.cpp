...
XboxCtrlNode::XboxCtrlNode() : Node("XboxController") {
	...
	auto joystick_callback = [this](const sensor_msgs::msg::Joy::SharedPtr msg) -> void {
		...
		// FIX (i) & (ii): JoystickAlgorithm is now called only inside the LB gate below,
		// so reverseOn and currentWheelAngle are only updated when LB is held.
		// We still compute the arrays here without side effects for use in button checks,
		// by reading axes directly — but reverseOn is NOT updated until LB is confirmed.
		...
		if (SendValBtn == 1) {
			// FIX (i) & (ii): JoystickAlgorithm (which updates reverseOn and currentWheelAngle)
			// is now called inside the LB gate, so state only changes when LB is held.
			...
			if ((savedArr > joystickArr[1] + 2 || savedArr < joystickArr[1] - 2) && gamepadArr[1] == 0 && (TurnRightBtn != 1 && TurnLeftBtn != 1)) {
				...
				TurnRightMotor = true;
				TurnLeftMotor = false;
				// FIX (iii): Was a while loop that only ever ran once — replaced with a plain if.
				// GetTurnMultiplier() flips the pivot values when wheels face the back 180°,
				// so the rover always physically turns right regardless of wheel orientation.
				if (!TurnedRight) {
					double m = GetTurnMultiplier();
					auto joystick_msg = std_msgs::msg::Float64MultiArray();
					joystick_msg.data = {-151.0 * m, 151.0 * m, 151.0 * m, -151.0 * m};
					joystickPub->publish(joystick_msg);
					savedArr = 600.0;
					TurnedRight = true;
				}
				...
				TurnRightMotor = false;
				// FIX (iii): Was a while loop that only ever ran once — replaced with a plain if.
				// FIX (iii): Copy-paste bug fixed — left turn now negates the right turn values.
				// GetTurnMultiplier() also applied here for the same orientation correction.
				if (!TurnedLeft) {
					double m = GetTurnMultiplier();
					auto joystick_msg = std_msgs::msg::Float64MultiArray();
					joystick_msg.data = {151.0 * m, -151.0 * m, -151.0 * m, 151.0 * m};
				...
				// FIX: Same while-loop-that-runs-once pattern replaced with plain if.
				if (!PivotHomed) {
...
std::vector<double> XboxCtrlNode::JoystickAlgorithm(double x_axis, double y_axis) {
	...
	if (x_axis == -0.0 && y_axis == -0.0) {
		// FIX (i) & (ii): Reset reverseOn when joystick is released so it does not
		// persist stale state into the next trigger_callback cycle.
		reverseOn = false;
		...
		// FIX (iii): Save the current wheel angle so GetTurnMultiplier() can use it.
		currentWheelAngle = val;
...
// FIX (iii): New helper — returns +1.0 if wheels face the front 180°, -1.0 if back 180°.
// This ensures TurnRight and TurnLeft pivot commands always produce the correct
// physical rotation regardless of which direction the wheels are currently pointing.
double XboxCtrlNode::GetTurnMultiplier() {
	// currentWheelAngle is in the range -250 to +250.
	// Front 180° maps to -125 to +125; back 180° is outside that range.
	if (currentWheelAngle >= -125.0 && currentWheelAngle <= 125.0) {
		return 1.0;
	} else {
		return -1.0;
	}
